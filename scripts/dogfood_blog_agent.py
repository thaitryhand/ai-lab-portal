"""Dogfood the AI Blog Agent pipeline with real OpenAI (semi-auto gates).

Usage (backend on :18000, Celery worker running, llm_e2e_fake=false):
  python scripts/dogfood_blog_agent.py
  python scripts/dogfood_blog_agent.py --skip-seed-project
"""
...
def seed_project() -> str:
    ...
    project_id = f"project_dogfood_{uuid.uuid4().hex[:8]}"
    slug = f"ai-lab-portal-dogfood-{uuid.uuid4().hex[:6]}"
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    PROJECT_PAYLOAD["project_summary"],
                    PROJECT_PAYLOAD["technical_highlights"],
                ),
            )
        conn.commit()
    finally:
        conn.close()
    print(f"Seeded published project {project_id} ({slug})")
    return project_id
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(
            f"Backend not reachable at {BACKEND_URL}. Start uvicorn + Celery worker first.\n{exc}"
        ) from exc

    if not args.skip_seed_project:
        seed_project()

    print("\n1. Generate idea (OpenAI)...", flush=True)
    if args.idea_id:
        idea_id = args.idea_id
        idea = load_idea(idea_id)
        print(f"   Resuming idea: {idea.get('title', idea_id)} ({idea_id})")
    else:
        status, result = api_call("POST", "/admin/blog-ideas/generate", PROJECT_PAYLOAD)
        if status == 202:
            task_id = (result.get("detail") or result).get("task_id")
            ...
            status, idea = api_call("GET", f"/admin/blog-ideas/{idea_id}")
            if status >= 300:
                raise RuntimeError(f"Could not load idea {idea_id}")
        elif 200 <= status < 300:
            idea = result
            idea_id = idea["id"]
        else:
